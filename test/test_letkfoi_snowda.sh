...
# set extra paths
OBSDIR=$project_binary_dir/test/ioda
...
# prepare yaml files
cp $project_source_dir/test/testinput/${DAtype}.yaml letkf_land.yaml
for ii in "${!OBS_TYPES[@]}";
do
    echo "============================= ${OBS_TYPES[$ii]}" 
    cat $project_source_dir/DA_update/jedi/fv3-jedi/yaml_files/${OBS_TYPES[$ii]}.yaml >> letkf_land.yaml
done
